...
func FindOtpAndExecuteQuery(w http.ResponseWriter, r *http.Request) {
	...
	var requestWeiget requestDtos.RequestWeiget
	decoder := json.NewDecoder(r.Body)
	err := decoder.Decode(&requestWeiget)
	if err != nil {
		logs.ErrorLogger.Println(err)
	}
	err = validations.ValidateQueryExecuter(requestWeiget)
	if err != nil {
		errors.BadRequest(w, err.Error())
		return
	} else {
		//find the weiget by Id and update query  filed
		result, err1 := nftcomposercontroller.FindWeigetById(requestWeiget.Id)
		if err1 != nil {
			ErrorMessage := err1.Error()
			errors.NoContent(w, ErrorMessage)
			return
		} else {
			//pass the weiget model to querylanguage service get result
			...
			if strings.HasPrefix(queryResult.Result, "\nError") {
				...
				_, err1 := nftcomposercontroller.FindWeigetAndUpdateQuery(requestWeiget)
				if err1 != nil {
